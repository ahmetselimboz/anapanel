...
@php
    ...
    $postImages = isset($extra->detail_images) && !blank($extra->detail_images) ? $extra->detail_images : $post->images;
@endphp
...
@section('meta')
    ...
    <link rel="canonical"
        href="{{ route('amppost', ['categoryslug' => $post->category->slug, 'slug' => $post->slug, 'id' => $post->id]) }}" />
    ...
    <meta property="og:image" content="{{ generateOgImage(imageCheck($post->images)) }}" />
    <meta property="og:url"
        content="{{ route('post', ['categoryslug' => $post->category->slug, 'slug' => $post->slug, 'id' => $post->id]) }}" />
    ...
    <meta name="twitter:image" content="{{ generateOgImage(imageCheck($post->images)) }}" />
                ...
                <div class="ad-block">
                    <a href="{{ adsCheck(8)->url }}" title="Reklam 8" class="externallink">
                        <img src="{{ asset(adsCheck(8)->images) }}" alt="Reklam 8" class="img-fluid lazy"
                            data-type="{{ adsCheck(8)->type }}" height="{{ adsCheck(8)->height }}"
                            width="{{ adsCheck(8)->width }}">
                    </a>
                </div>
            @endif
        </div>
        ...
        <div class="row infiniteContent">
            ...
            <div class="detay-social-buttons">
                ...
                @if ($magicbox['googlenews'] != null)
                    <div class="social-button"><a href="{{ $magicbox['googlenews'] }}"
                            title="{{ $magicbox['googlenews'] }}" class="externallink social-link shadow-sm "><i
                                class="google-news"></i></a></div>
                @endif
                <div class="social-button"><a
                        href="whatsapp://send?text={{ route('post', ['categoryslug' => $post->category->slug, 'slug' => $post->slug, 'id' => $post->id]) }}"
                        data-action="share/whatsapp/share" title="{{ $magicbox['googlenews'] }}"
                ...
                <div class="social-button"><a href="" title=""
                        class="externallink social-link shadow-sm btn-copy" id="copyDetail"><i class="copy-paste"></i></a>
                </div>
            ...
            <div class="col-12 col-lg-8">
                            ...
                            <div class="author-info border-end border-secondary pe-3" style="--bs-border-opacity: .3;">
                                <div class="author-avatar">
                                    <img src="{{ asset($post->author->avatar ?? '') }}"
                                        ...
                                        alt="{{ $post->author->name ?? '' }}" class="img-fluid">
                                </div>
                                <div>
                                    <h2 class="author-name mb-1">{{ $post->author->name ?? '' }}</h2>
                                    <p class="author-title">EDİTÖR</p>
                                </div>
                            </div>
                        @endif

                        <div class="d-flex flex-column align-items-center justify-content-center border-end border-secondary px-3"
                ...
                <img src="{{ imageCheck($postImages) }}" class="detayresim"
                ...
                <div class="detail-content mb-4" id="detailContent">
                    ...
                    @if (adsCheck(9) && adsCheck(9)->publish == 0)
                        ...
                        @else
                            <div class="ad-block">
                                <a href="{{ adsCheck(9)->url }}" title="Reklam {{ $ads9->id }}"
                                    class="externallink">
                    ...
                    @if (isset($extra->video_embed) and $extra->video_embed != null)
                        <div class="embed-responsive embed-responsive-16by9">
                            {!! str_replace('class="', 'class="embed-responsive-item ', $extra->video_embed) !!}</div>
                    @endif
                        ...
                        <blockquote cite="#" class="d-flex flex-wrap rounded my-4">
                            <div class="blockquote-image">
                                <a href="{{ route('post', ['categoryslug' => categoryCheck(relatedPostCheck($extra->related_news)->category_id)->slug, 'slug' => relatedPostCheck($extra->related_news)->slug, 'id' => relatedPostCheck($extra->related_news)->id]) }}"
                                    ...
                                    <img width="100"
                                        src="{{ imageCheck(relatedPostCheck($extra->related_news)->images) }}"
                ...
                <div class="w-100 mb-2 mb-md-4 d-block d-md-flex align-items-center">
                    <div class="d-flex justify-content-left detail-social-buttons col-12 col-md-6">
                        @if ($magicbox['googlenews'] != null)
                            <div class="social-button mb-4 mb-md-0"><a href="{{ $magicbox['googlenews'] }}"
                                    ...
                                    class="externallink social-link shadow-sm btn-google-news"><i
                                        class="google-news"></i></a>
                            </div>
                        @endif
                        <div class="social-button mb-4 mb-md-0"><a
                                href="whatsapp://send?text={{ route('post', ['categoryslug' => $post->category->slug, 'slug' => $post->slug, 'id' => $post->id]) }}"
                                data-action="share/whatsapp/share" title="{{ $magicbox['googlenews'] }}"
                                class="externallink social-link social-button-news-bottom shadow-sm btn-whatsapp">
                                <i class="whatsapp"></i>
                        ...
                        <div class="social-button mb-4 mb-md-0  position-relative">
                            <button type="button" title="" id="options-btn-area"
                            ...
                            <button type="button" title="" id="options-btn-area-2"
                ...
                @if ($extra->comment_status == 0)
                    ...
                    <div class="row d-none" id="yorumlar"><!--Yorumlar-->
                            ...
                            <div class="comment-block shadow-sm p-4 overflow-hidden rounded-1">
                                ...
                                <div class="comment-form">
                                    <form action="{{ route('commentsubmit', ['type' => 0, 'post_id' => $post->id]) }}"
                                        ...
                                        <div class="mb-3">
                                            @if (isset($magicbox['google_recaptcha_site_key']))
                                                <div class="g-recaptcha"
                                                    data-sitekey="{{ $magicbox['google_recaptcha_site_key'] }}"></div>
                                            @endif
                                        </div>
            ...
            <div class="col-12 ">
                ...
                @if (adsCheck(10) && adsCheck(10)->publish == 0)
                    <div class="col-12 mb-4">
                        ...
                        @else
                            <div class="ad-block">
                                <a href="{{ adsCheck(10)->url }}" title="Reklam {{ 10 }}"
                                    class="externallink">
                ...
                <!--                    <a href="{{ adsCheck(10)->url }}" title="Reklam {{ 10 }}"-->
                <!--                        class="externallink">-->
                <!--                        <img src="{{ asset('uploads/' . adsCheck(10)->images) }}"-->
                <!--                            alt="Reklam 10" class="img-fluid lazy"-->
                <!--                            data-type="{{ adsCheck(10)->type }}"-->
                <!--                            height="{{ adsCheck(10)->height }}"-->
                <!--                            width="{{ adsCheck(10)->width }}">-->
                <!--                    </a>-->
                <!--                </div>-->
                <!--            @endif-->
                <!--        </div>-->
...
@section('custom_js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#options-btn-area-2").click(function() {
                $(".options-card").toggleClass("d-none");
            })
        })
    </script>
