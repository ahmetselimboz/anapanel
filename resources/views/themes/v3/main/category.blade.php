...
@section('content')
    ...
    <div class="row">
		...
		<div class="col-12 col-md-8">
			...
			<div class="headline-block bg-white overflow-hidden rounded-1 shadow-sm mb-4">
				...
				@foreach($posts_slider as $key => $post)
				<button type="button" data-bs-target="#headlineCarousel" data-bs-slide-to="{{$key}}" @if($key==0) class="active" aria-current="true" @endif>{{$key+1}}</button>
				@if($key==14) @break @endif
				@endforeach
				</div>
				</div>
		...
		<div class="col-12 col-md-4">
			...
			<div class="mostly-block overflow-hidden rounded-1 shadow-sm">
				...
				<div class="tekli">
					<a href="{{ route('post', ['categoryslug'=>$hit_popup->category->slug,'slug'=>$hit_popup->slug,'id'=>$hit_popup->id]) }}" title="{{ html_entity_decode($hit_popup->title) }}">
		...
		<div class="spotlar">
			...
			@if($key<=4)
				...
				<div class="spot-2">
				<a href="{{ route('post', ['categoryslug'=>$post->category->slug,'slug'=>$post->slug,'id'=>$post->id]) }}" title="{{ html_entity_decode($post->title) }}">
				<p><span>{{html_entity_decode($post->title) }}</span></p>
				<img src="{{ route('resizeImage', ["i_url" => imageCheck($post->images), "w" => 216, "h" => 250]) }}" alt="{{ html_entity_decode($post->title) }}" />
				</a>
				</div>
    ...
    <div class="spotlar">
		...
		@if($counter<=6)
			...
			<div class="spot spotduz3 spotduz3-{{ $counter }}">
			<a href="{{ route('post', ['categoryslug'=>$post->category->slug,'slug'=>$post->slug,'id'=>$post->id]) }}" title="{{ html_entity_decode($post->title) }}">
			<img src="{{ route('resizeImage', ["i_url" => imageCheck($post->images), "w" => 380, "h" => 253]) }}" alt="{{ html_entity_decode($post->title) }}" />
			<p><span>{{ html_entity_decode($post->title) }}</span></p>
			</a>
			</div>
		...
		@else
			...
			<a href="{{ route('post', ['categoryslug'=>$post->category->slug,'slug'=>$post->slug,'id'=>$post->id]) }}" title="{{ html_entity_decode($post->title) }}">
			<img src="{{ route('resizeImage', ["i_url" => imageCheck($post->images), "w" => 380, "h" => 253]) }}" alt="{{$post['title']}}" alt="{{ html_entity_decode($post->title) }}" />
			<p><span>{{ html_entity_decode($post->title) }}</span></p>
			</a>
			</div>
